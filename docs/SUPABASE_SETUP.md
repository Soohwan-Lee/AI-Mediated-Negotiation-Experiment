...
SQL uses lowercase codes: `scf1` is SCF1, `bg2` is BG2, and so on. Background and study-wide codes are `bg1`–`bg7`, `fts1`–`fts3`, `aia1`–`aia5`, `rsc1`–`rsc4`, `icc1`–`icc3`, and the historical `oec1`. Canonical task-open answers live in `self_reports`, beside their actual task/mode/policy/role metadata. OEP applies only to the Proxy task. OET1 is optional; blank or missing values remain NULL and never prevent completion. End-study ICC and optional post-debrief comments remain in `study_participants`; comparison storage is versioned below.

The previous expanded task reflection instrument (v2) is recorded per task as `self_reports.open_instrument_version = '2.27-open-v2'`. Its common codes OED1/OEI1/OEF1/OEE1/OEN1/OER1 map directly to lowercase columns `oed1`/`oei1`/`oef1`/`oee1`/`oen1`/`oer1`. Proxy-only OEP1–OEP4 map to `oep1`–`oep4`; Direct rows leave these NULL. Task 2 additionally requires OEC1 in `self_reports.oec1`, mirrored to the historical participant comparison column. OET1 remains optional. Thus Task 1 requires 6 Direct or 10 Proxy text responses; Task 2 requires 7 Direct or 11 Proxy responses. All appear together after the role decision, with no minimum word count.

NULL or `2.27` task instrument markers retain the legacy OED1/OEE1/(Proxy OEP1) requirements. A legacy Task 2 still uses the comparison in wrap-up; new Task 2 already collected it and wrap-up contains only the existing end checks. Mixed-version sessions remain supported, old completed tasks are not upgraded, and a legacy write cannot downgrade a task already marked `2.27-open-v2`. The additive migration keeps existing data and assignment/write guards unchanged.

New task reflections now use `2.27-open-v3`. The four Proxy items are OEP1 (representation and control), OEP3 (own Proxy responsibility), OEP4 (other Proxy reason sources), and OEP5 (other Proxy responsibility). The single added nullable column is `self_reports.oep5`. OEP2 is not shown or newly saved in v3; its existing v2 control answers retain their original meaning and column. Six common items, Task-2-only OEC1, and optional OET1 are unchanged: 17 required reflections across each participant's two tasks. Direct OEP columns remain NULL.

Saved legacy/v2 drafts restore their exact instrument rather than being promoted. The API and SQL accept only known versions. Once a task reflection has started, its version is immutable in both directions, preventing old/new tabs from mixing question meanings. Historical NULL-marker rows containing reflection responses remain legacy. Completion requires OEP2 for v2 Proxy rows but OEP5 for v3 Proxy rows; both require OEP1/OEP3/OEP4. All nine pairs of task versions are regression-tested across the 16 assignments. Negotiation termination saves an agreed or unresolved task outcome; it does not set participation completion. The ordered save queue and authoritative SQL finalizer remain the separate final study-completion gate.
