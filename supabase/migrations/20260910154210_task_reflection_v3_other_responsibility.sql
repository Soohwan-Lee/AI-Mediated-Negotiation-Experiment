-- Preserve older instrument meanings and requirements; no historical rows change.
alter table public.self_reports add column oep5 text;
alter table public.self_reports add constraint self_reports_open_instrument_version_check
  check (open_instrument_version is null or open_instrument_version in ('2.27','2.27-open-v2','2.27-open-v3'));
comment on column public.self_reports.oep5 is 'Other Proxy responsibility, required only for Proxy tasks using 2.27-open-v3. OEP2 remains the historical v2 control item.';
comment on column public.self_reports.open_instrument_version is 'Per-task reflection instrument. NULL/2.27 legacy; 2.27-open-v2 preserves OEP2; 2.27-open-v3 combines representation/control in OEP1 and adds OEP5.';
...
begin
  if tg_table_name = 'self_reports' and tg_op = 'UPDATE' then
    if (old.open_instrument_version is not null and new.open_instrument_version is distinct from old.open_instrument_version)
      or (old.open_instrument_version is null and (
        old.open_submitted or num_nonnulls(old.oed1,old.oee1,old.oep1,old.oet1) > 0
        or old.responses ? ('v226_task_open_t' || old.task_index))
        and coalesce(new.open_instrument_version,'2.27') <> '2.27') then
      raise exception 'Task reflection instrument is immutable once started' using errcode = '23514';
    end if;
  end if;
...
begin
  ...
  for r in select * from public.self_reports where participant_key = p.participant_key loop
    if r.open_instrument_version in ('2.27-open-v2','2.27-open-v3') and (
      ...
      or (r.task_mode in ('user_specified', 'ai_supplemented') and (
        (r.open_instrument_version = '2.27-open-v2' and coalesce(btrim(r.oep2), '') = '')
        or (r.open_instrument_version = '2.27-open-v3' and coalesce(btrim(r.oep5), '') = '')
        or coalesce(btrim(r.oep3), '') = '' or coalesce(btrim(r.oep4), '') = ''))
    ...
    if r.task_index = 2 and coalesce(r.open_instrument_version,'2.27') = '2.27'
